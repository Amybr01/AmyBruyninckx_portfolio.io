document.addEventListener('DOMContentLoaded', () => {
  ...
  function makeDropPin(lat, lon, label, href) {
    ...
    pin.scale.set(1, 1, 1);
    // userData ook op de pin zelf
    pin.userData = { label, href };
    group.add(pin);

    // 6b) wit bolletje bovenop de pin
    ...
    // 6c) plaats het geheel net buiten de aardbol
    const pos = latLonToVector(lat, lon, R + 0.03);
    group.position.copy(pos);
    group.lookAt(earthMesh.position);

    // 6d) userData op de group zelf (voor de zekerheid)
